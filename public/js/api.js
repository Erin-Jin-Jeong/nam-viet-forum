...
// FIX: Import cả POSTS_API VÀ AUTH_API từ config.js
import { POSTS_API, AUTH_API } from "./config.js";
...
export async function deletePost(token, postId) {
  ...
  // Giữ nguyên logic cũ của bạn (chỉ trả về data)
  const data = await res.json();
  if (!res.ok) {
...
//  * @returns {Promise<object>} Đối tượng chứa JWT Token và Username. (ĐÃ THAY ĐỔI)
//  */
...
//     // FIX: Giả định API trả về { token: string, username: string }
//     if (!data.token || !data.username) {
//         throw new Error("Phản hồi API không hợp lệ: Thiếu token hoặc username.");
//     }
    
//     // TRẢ VỀ TOÀN BỘ ĐỐI TƯỢNG DATA (token và username)
//     return data; 
// }
 ...
 */
export async function authenticate(endpoint, credentials) {
  // Biến AUTH_API hiện đã được import ở đầu tệp
  const res = await fetch(`${AUTH_API}/${endpoint}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(credentials),
